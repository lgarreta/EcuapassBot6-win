...
class Extractor:
	...
	def getReplaceSubjectId (text, subject, replaceString, type):
		try:
			...
			reId     = rf"(RUC|NIT|OTROS|ID)[\s.:]+({reNumber}(?:-\d)?)\s*"
...
#		return (info)
	...
	def getValidValue (value):
		...
		try:
			if type (value) is not dict:
				return value if isValid (value) else None
			else:
				numInvalids = len ([v for k,v in value.items () if not isValid (v)])
	...
	def getMRNFromText (text):
		match = re.search(r"CEC\w*", text)
		if match:
			return (match.group())  # Output: CEC12345
		return None
		#reMRN = r"\bMRN\b\s*[:]?\s*\b(\S+)\b"
	...
	def getDate (text, resourcesPath=None):
		...
		try:
			...
			if option in [1, 2, 3, 5, 6]:
				month  = monthsList.index (month.upper()) + 1
